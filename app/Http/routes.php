...
Route::get('image/{encoded}', function ($encoded) {
    $string = base64_decode($encoded);

    $count = strlen($string);

    $width = (35 * $count * 0.7) + 75;

    if ($width < 400) {
        $width = 400;
    }

    $img = Image::canvas($width, 300, '#eeeeee');

    $fontSize = 75 - ($count * 0.3);

    $vertOffset = 115 + ($fontSize / 5) + 10;

    $img->text("We're Pivoting!", ($width / 2), 50, function ($font) use ($fontSize) {
        $font->file(public_path() . '/fonts/roboto/RobotoCondensed-Regular.ttf');
        $font->size(28);
        $font->color('#777777');
        $font->align('center');
        $font->valign('middle');
    ...
    $img->text($string, ($width / 2), $vertOffset, function ($font) use ($fontSize) {
        $font->file(public_path() . '/fonts/roboto/RobotoCondensed-Regular.ttf');
        $font->size($fontSize);
        $font->color('#444444');
        $font->align('center');
    ...
    $img->text("Generated By SaaSaaS from Tighten Co.\nhttp://saasaas.tighten.co/", ($width / 2), 225, function ($font) use ($fontSize) {
        $font->file(public_path() . '/fonts/roboto/RobotoCondensed-Regular.ttf');
        $font->size(16);
        $font->color('#777777');
        $font->align('center');
